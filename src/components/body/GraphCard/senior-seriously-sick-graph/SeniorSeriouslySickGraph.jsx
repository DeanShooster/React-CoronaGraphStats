...
import { GetSeniorSeriouslySick } from "../../../../api/DataFetches";


const SeniorSeriouslySickGraph = ( { width }) => {
    ...
    useEffect( ()=>{
        const fetchData = async ()=>{
            const result = await GetSeniorSeriouslySick();
            const finalResult = [];
            for(let i = 0; i < result.length; i++ )
                finalResult.push( {date: new Date(result[i].date), vaccinated: result[i].vaccinated,
...
export default SeniorSeriouslySickGraph;