...
import { GetVaccinatedVerified } from "../../../../api/DataFetches";
import CustomToolTip from "../custom-tool-tip/CustomToolTip";


const SeniorDailyVerifiedGraph = ( {width} ) => {
    ...
    useEffect( ()=>{
        const fetchData = async ()=>{
            const result = await GetVaccinatedVerified();
            const finalResult = [];
            for(let i = 0; i < result.length; i++ )
                finalResult.push( {date: new Date(result[i].date), completelyVaccinated: result[i].completelyVaccinated,
                    outdatedVaccinated: result[i].outdatedVaccinated, notVaccinated: result[i].notVaccinated } );
    ...
    return (
        <div>
            ...
            <LineChart width={width} height={250} data={data} margin={ {top: 25, right: 30, left: -5, bottom: 10, }}>
                ...
                <Line type="monotone" dataKey="completelyVaccinated" stroke='rgb(80, 203, 253)' strokeWidth={2.5} isAnimationActive={false}
...
export default SeniorDailyVerifiedGraph;