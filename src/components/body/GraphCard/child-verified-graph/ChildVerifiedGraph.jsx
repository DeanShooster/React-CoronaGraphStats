...
const ChildVerifiedGraph = ( {width} ) => {
    ...
    useEffect(()=>{
        const fetchData = async ()=> {
            const result = await GetVerifiedChildren();
            const finalResult = [];
    ...
    return (
        <div>
            ...
            <LineChart width={width} height={250} data={data} margin={ {top: 25, right: 30, left: -5, bottom: 10, }}>
                ...
                <YAxis dataKey='kid' axisLine={false} padding={ { bottom: 20 } } tick={{ dx:-30}} stroke={color}
...
export default ChildVerifiedGraph;