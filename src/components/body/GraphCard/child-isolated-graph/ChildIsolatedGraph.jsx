...
import { GetIsolatedChildren } from "../../../../api/DataFetches";


const ChildIsolatedGraph = ( {width} ) => {
    ...
    useEffect(()=>{
        const fetchData = async ()=> {
            const result = await GetIsolatedChildren();
            const finalResult = [];
        ...
        <div>
            ...
            <LineChart width={width} height={250} data={data} margin={ {top: 25, right: 30, left: -5, bottom: 10, }}>
                ...
                <YAxis dataKey='infant' axisLine={false} padding={ { bottom: 20 } } tick={{ dx:-30}} stroke={color}
                    label={ {value:'מבודדים', position: 'top', offset:15, fill: color}} tickLine={{ stroke: 'rgba(255, 0, 0, 0)'} }/>
...
export default ChildIsolatedGraph;